...
```{r}
deserts$year <- as.factor(deserts$year) #R expresses "year" as numeric, change year to facor
```
...
deserts %>% 
  ...
  geom_line()+
  geom_point(shape=2)+ #add custom points
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "black", fill = "lemonchiffon", bins=40)+
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "black", fill = "royalblue", bins=40)+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x = log10.hra)) +
  geom_density(fill="midnightblue", alpha  =0.4, color = "black")+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x=log10.hra)) +
  geom_histogram(aes(y = ..density..), fill = "mediumseagreen", alpha = 0.4, color = "black")+
