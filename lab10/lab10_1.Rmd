...
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass=mean(mass, na.rm=T)) %>% 
  ggplot(aes(x=reorder(order, mean_mass), y=mean_mass))+geom_col()+coord_flip()+scale_y_log10() #scale_y_log10() creates a graph with close results; can get better idea #reorder lines up bars in order 
```
...
#theme adjusts various elements in the plot (e.g. font size, etc.)
```
...
elephants %>% 
  ...
  labs(title = "Elephant Age vs. Height",
       x = "Age",
       y = "Height") +
  theme(plot.title = element_text(size = rel(2), hjust = 1)) #rel = relative size, hjust = horizontal adjustment
```
...
#fill will fill in specified variables; adds color by (e.g. sex); colors plot plus keys
```
...
homerange %>% 
  ...
  geom_point(size=1.5, color="red", alpha=0.5)+ #alpha = make points transparent
