...
ui <- fluidPage(
    ...
    sliderInput("pointsize", "Select the Point Size", min = 0.5, max = 3, value = 2, step = 0.5),
...
ui <- dashboardPage(
  ...
  fluidRow(
  box(title = "Plot Options", width = 3,
  selectInput("y", "Select Y Variable", choices = c("sex"), 
              selected = "sex"),
...
server <- function(input, output, session) { 
  ...
  wolves %>% 
      filter(sex!"NA") %>% 
      filter(pop)
  ggplot(penguins, aes_string(x = input$x, y = input$y, color="sex")) +
      geom_point(size=input$pointsize, alpha=0.8) + 
      theme_light(base_size = 18)
  })
  session$onSessionEnded(stopApp)
